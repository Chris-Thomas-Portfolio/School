...
int main(void)
{
	...
	DDRD = DDRD | 0xC0;//PD7(output)->segment G; PD6(output)->buzzer; PD5(input)->switch
	//DDRD = (DDRD & 0x20);//sets PD5 as an input leaves other bits unchanged
	PORTD=PORTD | 0x20;//turns on pullup resistor connected to pd5
	_delay_ms(1);
	while((PIND & 0x20)){}//wait for button to be pressed
	...
	PORTD =(PORTD | 0x40);		//turn on active buzzer
	_delay_ms(1000);			//wait for one second
	PORTD = (PORTD & 0xBF);		//turn off active buzzer
